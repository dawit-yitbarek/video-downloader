...
export async function refreshCookies(dir, url, outPath) {
    ...
    try {
        ...
        try {
            ...
            console.warn(`⚠️ Navigation incomplete for ${url}:`, err.message || err);
        }
        ...
        if (!cookies || cookies.length === 0) {
            console.warn(`⚠️ Extracted 0 cookies for ${url}. Existing cookie file preserved.`);
            return;
        }
        ...
        if (browser) {
            try {
                ...
                console.warn("⚠️ Browser close failed:", e.message || e);
            }
        }
    }
}
...
export async function firstLogin(dir, url) {
    ...
    try {
        ...
        console.log(`👉 Please log in manually in the opened ${url} window. Close the browser when finished.`);

        await new Promise((resolve) => browser.on("disconnected", resolve));

        console.log("✅ Browser closed — session (profile) saved locally.");
    } catch (err) {
        console.error(`❌ Failed to start first login for ${url}:`, err);
    } finally {
        if (browser) {
            try {
                await browser.close();
